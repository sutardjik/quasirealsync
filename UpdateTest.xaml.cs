...
namespace mysqlpostgres
{
    ...
    public partial class UpdateTest : Window
    {
        ...
        private void submit_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                MySqlCommand cmd = new MySqlCommand("UPDATE persons SET LastName='" + lastname.Text + "', FirstName='" + firstname.Text + "' WHERE PersonID='" + personid.Text + "'", conn);
                if (cmd.ExecuteNonQuery() == 1)
                {
                    MessageBox.Show("Updated row in MySQL persons table.");
                    conn.Close();
                }
                else
                {
                    MessageBox.Show("Failed to update row in MySQL persons table.");
                }
            }
            catch (Exception ex)
            {
