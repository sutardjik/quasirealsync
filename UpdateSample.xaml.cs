...
{
    /// <summary>
    /// Interaction logic for UpdateSample.xaml
    /// </summary>
    public partial class UpdateSample : Window
    {
        public UpdateSample()
        {
            InitializeComponent();
        ...
        private void submit_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                MySqlCommand cmd = new MySqlCommand("UPDATE employees SET LastName='" + lastname.Text + "', FirstName='" + firstname.Text + "' WHERE EmployeeID='" + employeeid.Text + "'", conn);
                if (cmd.ExecuteNonQuery() == 1)
                {
                    MessageBox.Show("Updated row in MySQL employees table.");
                    conn.Close();
                }
                else
                {
                    MessageBox.Show("Failed to update row in MySQL employees table.");
                }
            }
            catch (Exception ex)
            {
