...
namespace mysqlpostgres
{
    ...
    public partial class DeleteTest : Window
    {
        ...
        private void submit_Click(object sender, RoutedEventArgs e)
        {
            ...
            conn.Open();
            MySqlCommand cmd = new MySqlCommand("DELETE FROM persons WHERE PersonID='" + personid.Text + "'", conn);
            try
            {
                if (cmd.ExecuteNonQuery() == 1)
                {
                    MessageBox.Show("Deleted from MySQL persons table.");
                }
                else
                {
                    MessageBox.Show("Failed to delete from MySQL persons table.");
                }
            }
            catch (Exception ex)
            {
