...
{
    /// <summary>
    /// Interaction logic for DeleteSample.xaml
    /// </summary>
    public partial class DeleteSample : Window
    {
        public DeleteSample()
        {
            InitializeComponent();
        ...
        private void submit_Click(object sender, RoutedEventArgs e)
        {
            ...
            conn.Open();
            MySqlCommand cmd = new MySqlCommand("DELETE FROM employees WHERE EmployeeID='" + employeeid.Text + "'", conn);
            try
            {
                if (cmd.ExecuteNonQuery() == 1)
                {
                    MessageBox.Show("Deleted from MySQL employees table.");
                }
                else
                {
                    MessageBox.Show("Failed to delete from MySQL employees table.");
                }
            }
            catch (Exception ex)
            {
