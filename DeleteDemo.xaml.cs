...
{
    /// <summary>
    /// Interaction logic for DeleteDemo.xaml
    /// </summary>
    public partial class DeleteDemo : Window
    {
        public DeleteDemo()
        {
            InitializeComponent();
        ...
        private void submit_Click(object sender, RoutedEventArgs e)
        {
            ...
            conn.Open();
            MySqlCommand cmd = new MySqlCommand("DELETE FROM customers WHERE CustomerID='" + customerid.Text + "'", conn);
            try
            {
                if (cmd.ExecuteNonQuery() == 1)
                {
                    MessageBox.Show("Deleted row from MySQL customers table.");
                }
                else
                {
                    MessageBox.Show("Failed to delete row from MySQL customers table.");
                }
            }
            catch (Exception ex)
            {
