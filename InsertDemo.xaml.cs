...
{
    /// <summary>
    /// Interaction logic for InsertDemo.xaml
    /// </summary>
    public partial class InsertDemo : Window
    {
        public InsertDemo()
        {
            InitializeComponent();
        ...
        {
            string connString = "server=localhost; uid=root; pwd=test123;database=demo";
            MySqlConnection conn = new MySqlConnection(connString);
            conn.Open();
            MySqlCommand cmd = new MySqlCommand("INSERT INTO customers(CustomerID,LastName,FirstName) VALUES ('" + customerid.Text + "','" + lastname.Text + "','" + firstname.Text + "')", conn);
            try
            {
                if (cmd.ExecuteNonQuery() == 1)
                {
                    MessageBox.Show("Inserted row into MySQL customers table.");
                }
                else
                {
                    MessageBox.Show("Failed to insert row into MySQL customers table.");
                }
            }
            catch (Exception ex)
            {
